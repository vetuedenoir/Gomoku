...
{
	const char* who = aiVsAi ? (d.chooser == Seat::First ? "Seat 1" : "Seat 2") : "AI";
...
Gomoku::Gomoku() : _window(sf::VideoMode(WIN_W, WIN_H), "Gomoku", sf::Style::Titlebar | sf::Style::Close)
{
...
MenuPage& Gomoku::currentPage()
{
	switch (_states.top())
	{
		case AppState::RuleDemos:
			return _ruleDemos;
		case AppState::BoardSize:
			return _boardSize;
		case AppState::StoneColor:
			return _stoneColor;
...
void Gomoku::logConfig() const
{
	...
	const char* modeStr      = _config.aiVsAi ? "AI vs AI" : _config.aiOpponent ? "vs AI" : "hotseat";

	LOG_INFO("CONFIG", std::string("mode=") + modeStr + "  board=" + std::to_string(_config.boardSize) + "x" +
	                       std::to_string(_config.boardSize) +
	                       "  stone=" + stoneStr[static_cast<int>(_config.playerColor)] +
	                       "  opening=" + openingStr[static_cast<int>(_config.openingProtocol)]);
	LOG_SUPPRESS(modeStr, stoneStr[static_cast<int>(_config.playerColor)],
	             openingStr[static_cast<int>(_config.openingProtocol)]);
}
...
void Gomoku::startGame()
{
	...
	LOG_INFO("GAME DEBUG", "player actor: " + seatStr(_controller->playerActor().seat) + " " +
	                           (_controller->playerActor().color == Color::Black ? "Black" : "White"));
	LOG_INFO("GAME DEBUG", "ai actor: " + seatStr(_controller->aiActor().seat) + " " +
	                           (_controller->aiActor().color == Color::Black ? "Black" : "White"));
	LOG_INFO("GAME DEBUG", "current actor: " + seatStr(_controller->currentActor().seat) + " " +
	                           (_controller->currentActor().color == Color::Black ? "Black" : "White"));
}
...
CellStatus Gomoku::computeGhostColor() const
{
	...
	switch (_controller->phase())
	{
		...
		case GamePhase::Standard:
			return (_controller->currentColor() == Color::Black) ? CellStatus::Black : CellStatus::White;
...
bool Gomoku::isGameOver()
{
	...
	buildWinScreenPage(winner.value(), _controller->blackCaptureCount(), _controller->whiteCaptureCount());
...
void Gomoku::announceAiOpeningDecision()
{
	...
	_statusBanner.flash(formatOpeningDecision(*decision, _controller->aiVsAi()), GOLD, OPENING_DECISION_SECONDS);
}
...
void Gomoku::refreshStatusBanner()
{
	...
	if (_controller->phase() == GamePhase::Standard && !_controller->aiVsAi())
	{
		...
		_statusBanner.setPersistent(active ? "Suggested move shown  |  press H for another"
...
void Gomoku::buildColorChoicePage()
{
	...
	const bool            threeOptions =
		(openingProtocol == OpeningProtocol::Swap2 && seat == Seat::Second && _controller->stepIdx() == 1);

	const char* titleStr = threeOptions
	                           ? "Seat 2: Choose your option"
	                           : (seat == Seat::Second ? "Seat 2: Choose your colour" : "Seat 1: Choose your colour");
	...
	_colorChoice.addItem("opt1", FonctionItem(Item(label1, _font, CX, WIN_H * 0.545f),
	                                          [this]() { _controller->resolveColorChoice(false); }));
	_colorChoice.addItem("opt2", FonctionItem(Item(label2, _font, CX, WIN_H * 0.614f),
	                                          [this]() { _controller->resolveColorChoice(true); }));
	if (threeOptions)
	{
		_colorChoice.addItem("place2", FonctionItem(Item("Place 2 More", _font, CX, WIN_H * 0.6825f),
	...
	_colorChoice.setDrawFunction(
		[](MenuPage& page, sf::RenderWindow& win)
		{
			if (auto* t = page.getText("title"))
				win.draw(*t);
			if (auto* r = page.getRectangle("divider"))
				win.draw(*r);
			if (auto* fi = page.getItem("opt1"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("opt2"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("place2"))
				fi->item.draw(win);
		});
}
...
void Gomoku::buildWinScreenPage(const Color winner, int capturesBlack, int capturesWhite)
{
	...
	std::string scoreStr = "Black: " + std::to_string(capturesBlack / 2) +
	                       " cap  |  "
	                       "White: " +
	                       std::to_string(capturesWhite / 2) + " cap";
	...
	if (_config.aiOpponent)
	{
		...
		if (_config.aiVsAi)
		{
			avgStream << "Black avg: " << _controller->aiMoveAverageMs(Color::Black)
					  << " ms  |  White avg: " << _controller->aiMoveAverageMs(Color::White) << " ms";
		}
		else
		{
			avgStream << "AI avg: " << _controller->aiMoveAverageMs(_controller->aiActor().color) << " ms";
		}
	...
	_winScreen.addItem("again",
	                   FonctionItem(Item("Play Again", _font, CX, WIN_H * 0.5375f), [this]() { resetToMainMenu(); }));
	_winScreen.addItem("quit", FonctionItem(Item("Quit", _font, CX, WIN_H * 0.6375f), [this]() { _window.close(); }));

	_winScreen.setDrawFunction(
		[](MenuPage& page, sf::RenderWindow& win)
		{
			if (auto* t = page.getText("title"))
				win.draw(*t);
			if (auto* t = page.getText("score"))
				win.draw(*t);
			if (auto* t = page.getText("aiAvg"))
				win.draw(*t);
			if (auto* fi = page.getItem("again"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("quit"))
				fi->item.draw(win);
		});
}
...
void Gomoku::buildMainMenuPage()
{
	...
	_mainMenu.addItem("playAI", FonctionItem(Item("Play vs AI", _font, CX, WIN_H * 0.375f),
	                                         [this]()
	                                         {
	...
	_mainMenu.addItem("playHotseat", FonctionItem(Item("Two Players", _font, CX, WIN_H * 0.460f),
	                                              [this]()
	                                              {
													  _config.aiOpponent = false;
													  _config.aiVsAi     = false;
	...
	_mainMenu.addItem("playAIvsAI", FonctionItem(Item("AI vs AI", _font, CX, WIN_H * 0.545f),
	                                             [this]()
	                                             {
													 _config.aiOpponent  = true;
													 _config.aiVsAi      = true;
	...
	_mainMenu.addItem("quit", FonctionItem(Item("Quit", _font, CX, WIN_H * 0.630f), [this]() { _window.close(); }));

	_mainMenu.setDrawFunction(
		[](MenuPage& page, sf::RenderWindow& win)
		{
			if (auto* t = page.getText("title"))
				win.draw(*t);
			if (auto* r = page.getRectangle("divider"))
				win.draw(*r);
			if (auto* t = page.getText("sub"))
				win.draw(*t);
			if (auto* fi = page.getItem("playAI"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("playHotseat"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("playAIvsAI"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("quit"))
				fi->item.draw(win);
		});
...
void Gomoku::buildRuleDemosPage()
{
	...
	_ruleDemos.addItem("newGame", FonctionItem(Item("New game", _font, CX, WIN_H * 0.30f),
	                                           [this]()
	                                           {
												   clearRuleDemo();
												   navigateTo(AppState::BoardSize);
											   }));
	_ruleDemos.addItem("doubleThree", FonctionItem(Item("Double-three", _font, CX, WIN_H * 0.38f),
	                                               [this]() { startRuleDemo(demoDoubleThree()); }));
	_ruleDemos.addItem("doubleThreeCap", FonctionItem(Item("Double-three + capture", _font, CX, WIN_H * 0.46f),
	                                                  [this]() { startRuleDemo(demoDoubleThreeCapture()); }));
	_ruleDemos.addItem("breakableFive", FonctionItem(Item("Breakable five", _font, CX, WIN_H * 0.54f),
	                                                 [this]() { startRuleDemo(demoBreakableFive()); }));
	_ruleDemos.addItem("return", FonctionItem(Item("Return", _font, CX, WIN_H * 0.64f), [this]() { goBack(); }));

	_ruleDemos.setDrawFunction(
		[](MenuPage& page, sf::RenderWindow& win)
		{
			if (auto* t = page.getText("title"))
				win.draw(*t);
			if (auto* t = page.getText("sub"))
				win.draw(*t);
			if (auto* fi = page.getItem("newGame"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("doubleThree"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("doubleThreeCap"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("breakableFive"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("return"))
				fi->item.draw(win);
		});
}
...
void Gomoku::buildBoardSizePage()
{
	...
	_boardSize.addItem(
		"15x15", FonctionItem(Item("15 x 15", _font, CX, WIN_H * 0.3375f), [this]() { onBoardSizeSelected(15); }));
	_boardSize.addItem("19x19",
	                   FonctionItem(Item("19 x 19", _font, CX, WIN_H * 0.444f), [this]() { onBoardSizeSelected(19); }));
	_boardSize.addItem("return", FonctionItem(Item("Return", _font, CX, WIN_H * 0.5625f), [this]() { goBack(); }));
	_boardSize.addItem("quit", FonctionItem(Item("Quit", _font, CX, WIN_H * 0.6625f), [this]() { _window.close(); }));

	_boardSize.setDrawFunction(
		[](MenuPage& page, sf::RenderWindow& win)
		{
			if (auto* t = page.getText("title"))
				win.draw(*t);
			if (auto* t = page.getText("sub"))
				win.draw(*t);
			if (auto* fi = page.getItem("15x15"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("19x19"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("return"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("quit"))
...
void Gomoku::buildStoneColorPage()
{
	...
	_stoneColor.addItem("white", FonctionItem(Item("White", _font, CX, WIN_H * 0.3375f),
	                                          [this]() { onStoneColorSelected(Color::White); }));
	_stoneColor.addItem("black", FonctionItem(Item("Black", _font, CX, WIN_H * 0.4375f),
	                                          [this]() { onStoneColorSelected(Color::Black); }));
	_stoneColor.addItem("random", FonctionItem(Item("Random", _font, CX, WIN_H * 0.5375f),
	                                           [this]()
	                                           {
	...
	_stoneColor.addItem(
		"return", FonctionItem(Item("Return", _font, CX - WIN_W * 0.14f, WIN_H * 0.644f), [this]() { goBack(); }));
	_stoneColor.addItem(
		"quit", FonctionItem(Item("Quit", _font, CX + WIN_W * 0.14f, WIN_H * 0.644f), [this]() { _window.close(); }));

	_stoneColor.setDrawFunction(
		[](MenuPage& page, sf::RenderWindow& win)
		{
			if (auto* t = page.getText("title"))
				win.draw(*t);
			if (auto* t = page.getText("sub"))
				win.draw(*t);
			if (auto* t = page.getText("note"))
				win.draw(*t);
			if (auto* fi = page.getItem("white"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("black"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("random"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("return"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("quit"))
...
void Gomoku::buildOpeningPage()
{
	...
	_opening.addItem("Standard", FonctionItem(Item("Standard", _font, col1, WIN_H * 0.275f),
	                                          [this]() { onOpeningProtocolSelected(OpeningProtocol::Standard); }));
	_opening.addItem("pro", FonctionItem(Item("Pro", _font, col2, WIN_H * 0.275f),
	                                     [this]() { onOpeningProtocolSelected(OpeningProtocol::Pro); }));
	_opening.addItem("longpro", FonctionItem(Item("Long Pro", _font, col1, WIN_H * 0.375f),
	                                         [this]() { onOpeningProtocolSelected(OpeningProtocol::LongPro); }));
	_opening.addItem("swap", FonctionItem(Item("Swap", _font, col2, WIN_H * 0.375f),
	                                      [this]() { onOpeningProtocolSelected(OpeningProtocol::Swap); }));
	_opening.addItem("swap2", FonctionItem(Item("Swap 2", _font, CX, WIN_H * 0.475f),
	                                       [this]() { onOpeningProtocolSelected(OpeningProtocol::Swap2); }));
	_opening.addItem("return", FonctionItem(Item("Return", _font, col1, WIN_H * 0.6125f), [this]() { goBack(); }));
	_opening.addItem("quit", FonctionItem(Item("Quit", _font, col2, WIN_H * 0.6125f), [this]() { _window.close(); }));

	_opening.setDrawFunction(
		[](MenuPage& page, sf::RenderWindow& win)
		{
			if (auto* t = page.getText("title"))
				win.draw(*t);
			if (auto* t = page.getText("sub"))
				win.draw(*t);
			if (auto* fi = page.getItem("Standard"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("pro"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("longpro"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("swap"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("swap2"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("return"))
				fi->item.draw(win);
			if (auto* fi = page.getItem("quit"))
