...
static void drawPlayerCard(sf::RenderWindow& w, const sf::Font& font, float cx, float cy, float cardW, float cardH,
                           const std::string& colorName, const std::string& role, sf::Color stoneColor,
                           int capturePairs, int maxPairs, bool isActive, bool isAI, double aiLastMs, double aiAvgMs)
{
	...
	sf::Text caps = makeText(std::to_string(capturePairs) + " / " + std::to_string(maxPairs), font, FONT_MD, WHITE);
...
void UIRenderer::renderGame(sf::RenderWindow& w, Board& board, const IGameController& ctrl, CellStatus ghost,
...
void UIRenderer::renderStats(sf::RenderWindow& w, const sf::Font& font, const Board& board, const IGameController& ctrl)
{
	...
	const std::string blackRole = aiVsAi ? "AI" : hotseat ? "P1" : (playerIsBlack ? "YOU" : "AI");
	const std::string whiteRole = aiVsAi ? "AI" : hotseat ? "P2" : (playerIsBlack ? "AI" : "YOU");
	...
	// Black on the left
	drawPlayerCard(w, font, leftCX, cy, cardW, cardH, "Black", blackRole, STONE_BLACK, blackPairs, maxPairs,
	               turn == Color::Black, blackIsAI, ctrl.aiMoveLastMs(Color::Black),
	               ctrl.aiMoveAverageMs(Color::Black));

	// White on the right
	drawPlayerCard(w, font, rightCX, cy, cardW, cardH, "White", whiteRole, STONE_WHITE, whitePairs, maxPairs,
	               turn == Color::White, whiteIsAI, ctrl.aiMoveLastMs(Color::White),
	               ctrl.aiMoveAverageMs(Color::White));
}
