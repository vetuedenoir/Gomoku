...
{
    item_map.emplace(key, item);
}

void MenuPage::removeText(const std::string &key)      { text_map.erase(key); }
void MenuPage::removeRectangle(const std::string &key) { rectangle_map.erase(key); }
void MenuPage::removeItem(const std::string &key)      { item_map.erase(key); }
...
bool MenuPage::hasText     (const std::string &key) const { return text_map.find(key)      != text_map.end(); }
bool MenuPage::hasRectangle(const std::string &key) const { return rectangle_map.find(key) != rectangle_map.end(); }
bool MenuPage::hasItem     (const std::string &key) const { return item_map.find(key)       != item_map.end(); }
