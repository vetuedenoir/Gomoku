...
static OpeningStep makeStep(Seat actor, std::initializer_list<StoneSpec> stones, bool triggersChoice = false)
{
	OpeningStep s;
	s.actor               = actor;
...
static bool isLegalPlacement(int col, int row, int boardSize, const PlacementConstraint& c,
...
static std::string openingStepDescription(OpeningProtocol openingProtocol, int stepIdx)
{
	switch (openingProtocol)
	{
		case OpeningProtocol::Pro:
			...
			{
				case 0:
					return "Pro  step 0/2 — TentativeFirst places 1 Black stone at the board centre (forced)";
				case 1:
					return "Pro  step 1/2 — TentativeSecond places 1 White stone anywhere on the board";
				case 2:
					return "Pro  step 2/2 — TentativeFirst places 1 Black stone "
		...
		case OpeningProtocol::LongPro:
			...
			{
				case 0:
					return "LongPro  step 0/2 — TentativeFirst places 1 Black stone at the board centre (forced)";
				case 1:
					return "LongPro  step 1/2 — TentativeSecond places 1 White stone anywhere on the board";
				case 2:
					return "LongPro  step 2/2 — TentativeFirst places 1 Black stone "
		...
		case OpeningProtocol::Swap2:
			...
			{
				case 0:
					return "Swap2  step 0/1 — TentativeFirst places 3 stones (Black, Black, White) anywhere; "
						   "TentativeSecond may (A) play White, (B) swap to Black, "
						   "or (C) place 2 more stones and pass the choice back";
				case 1:
					return "Swap2  step 1/1 — TentativeSecond places 2 stones (Black, White) anywhere "
...
OpeningEngine::OpeningEngine(OpeningProtocol protocol) : _protocol(protocol), _steps(buildOpeningSteps(protocol))
{
	LOG_INFO("OPENING", std::string("protocol set") + "  steps=" + std::to_string(_steps.size()));
...
bool OpeningEngine::canPlace(const GameBoard& board, const Move& move) const
{
	...
	if (!isLegalPlacement(move.col, move.row, board.getSize(), spec.constraint, _history))
		return false;
...
OpeningCommitResult OpeningEngine::commit(GameBoard& board, const Move& move)
{
	...
	const std::string stepInfo =
		"step " + std::to_string(_stepIdx) + "/" + std::to_string(static_cast<int>(_steps.size()) - 1) + "  sub " +
		std::to_string(_subIdx) + "/" + std::to_string(static_cast<int>(step.stones.size()) - 1) +
	...
	if (!canPlace(board, move))
	{
		if (move.forcedColor != CellStatus::Empty && move.forcedColor != spec.color)
		{
			LOG_WARN("OPENING", stepInfo + " | wrong colour: got " + colorStr(move.forcedColor) + ", expected " +
			                        colorStr(spec.color));
		...
		else
		{
			if (!board.isFree(move.col, move.row))
			{
				LOG_WARN("OPENING", stepInfo + " | (" + std::to_string(move.col) + "," + std::to_string(move.row) +
				                        ") cell occupied");
			...
			if (spec.constraint.mustBeCenter)
				...
				reason = "too close to stone[" + std::to_string(spec.constraint.refStoneIdx) + "] (min dist " +
				         std::to_string(spec.constraint.minDist) + ")";
			else
	...
	LOG_DEBUG("OPENING", stepInfo + " | " + colorStr(spec.color) + " → (" + std::to_string(move.col) + "," +
...
std::vector<Move> OpeningEngine::legalMoves(const GameBoard& board) const
{
	...
	LOG_DEBUG("OPENING", "legalMoves: " + std::to_string(moves.size()) + " candidates");
...
{
	LOG_INFO("CHOICE", "Seat::Second chose option 3 — placing 2 more stones (B + W)");
	_subIdx = 0;
}
