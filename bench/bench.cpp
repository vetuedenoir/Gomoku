...
namespace
{
	...
	private:
		...
		t_cell parseMove()
		{
			...
			while (true)
			{
				...
				expect(':');
				if (key == "x")
					m.x = static_cast<int_fast16_t>(parseInt());
				else if (key == "y")
					m.y = static_cast<int_fast16_t>(parseInt());
				else
					skipValue();
			}
			return m;
		...
		BenchStats parseStats()
		{
			...
			while (true)
			{
				...
				if (key == "nodesVisited")
					s.nodesVisited = v;
				else if (key == "nodesEvaluated")
					s.nodesEvaluated = v;
				else if (key == "nodesPruned")
					s.nodesPruned = v;
				else if (key == "maxDepthSeen")
					s.maxDepthSeen = v;
				else if (key == "ttHits")
					s.ttHits = v;
				else if (key == "ttCutoffs")
					s.ttCutoffs = v;
				else if (key == "ttStores")
					s.ttStores = v;
				else if (key == "ttOrderingHits")
					s.ttOrderingHits = v;
				else if (key == "ttRootHits")
					s.ttRootHits = v;
				else if (key == "ttRootOrderingHits")
					s.ttRootOrderingHits = v;
				else if (key == "ttRootExactSeeds")
					s.ttRootExactSeeds = v;
				else if (key == "forcedNodes")
					s.forcedNodes = v;
			}
			return s;
		}
		...
		BenchResult parseResult()
		{
			...
			while (true)
			{
				...
				if (key == "name")
					...
					r.bestMove = parseMove();
				else if (key == "bestScore")
					r.bestScore = static_cast<int>(parseInt());
				else if (key == "timeUs")
					r.timeUs = parseInt();
				else if (key == "stats")
					r.stats = parseStats();
	...
	RunReport runBench(int depth, std::vector<BenchPosition> positions)
	{
		...
		for (size_t i = 0; i < positions.size(); ++i)
		{
			...
			std::cerr << "[" << std::setw(3) << (i + 1) << "/" << std::setw(3) << positions.size() << "] " << std::left
					  << std::setw(22) << p.name << std::right << "  move=(" << static_cast<int>(move.x) << ","
					  << static_cast<int>(move.y) << ")"
					  << "  score=" << st.bestScore << "  " << std::fixed << std::setprecision(1) << ms << " ms"
	...
	int compareReports(const RunReport& ref, const RunReport& neu)
	{
		...
		for (size_t i = 0; i < n; ++i)
		{
			...
			{
				std::ostringstream ss;
				ss << "  - " << a.name << "  ref=(" << static_cast<int>(a.bestMove.x) << ","
				   << static_cast<int>(a.bestMove.y) << ") score=" << a.bestScore << "  new=("
				   << static_cast<int>(b.bestMove.x) << "," << static_cast<int>(b.bestMove.y)
				   << ") score=" << b.bestScore;
		...
		std::cout << "Exactitude   : " << exact << "/" << n << " identiques (bestMove + bestScore)     "
				  << mark(exact == static_cast<int>(n) && ref.positions.size() == neu.positions.size()) << "\n";
		std::cout << "Nœuds        : " << groupThousands(nodesRef) << " → " << groupThousands(nodesNew) << "   "
				  << (nodesOk ? "(identique)"
		                      : ("(" + pctStr(static_cast<double>(nodesRef), static_cast<double>(nodesNew)) + ")"))
				  << "           " << mark(nodesOk) << "\n";
		std::cout << "Temps        : " << std::fixed << std::setprecision(2) << secRef << " s → " << secNew
				  << " s        (" << pctStr(secRef, secNew) << ")\n";
		std::cout << "Nœuds/s      : " << groupThousands(static_cast<int64_t>(npsRef / 1000.0)) << " k → "
				  << groupThousands(static_cast<int64_t>(npsNew / 1000.0)) << " k           (" << pctStr(npsRef, npsNew)
				  << ")\n";

		if (ref.positions.size() != neu.positions.size())
		{
			std::cout << "\nTaille des suites : " << ref.positions.size() << " → " << neu.positions.size()
					  << " (différent)\n";
		}
		if (!mismatches.empty())
		{
		...
		const bool ok = exact == static_cast<int>(n) && ref.positions.size() == neu.positions.size() && nodesOk;
		return ok ? 0 : 1;
	}
