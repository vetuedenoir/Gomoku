...
template<typename Traits> class TurnController
{
public:
	TurnOutcome<Traits> play(t_BWBoard<Traits>& bb, const Move& move, int capturesBlack, int capturesWhite,
...
template<typename Traits> Color TurnController<Traits>::colorFromCell(CellStatus cell)
{
	return (cell == CellStatus::Black) ? Color::Black : Color::White;
}

template<typename Traits> const char* TurnController<Traits>::colorLabel(const Color color)
{
...
{
	LOG_DEBUG("TURN", std::string(colorLabel(color)) + " → (" + std::to_string(move.col) + "," +
...
template<typename Traits>
TurnOutcome<Traits> TurnController<Traits>::play(t_BWBoard<Traits>& bb, const Move& move, int capturesBlack,
                                                 int capturesWhite, const std::optional<PendingWin>& pending) const
{
	...
	const FiveVerdict verdict = judgeFiveAfterMove<Traits>(bb, color, move.col, move.row, defenderCaptures);
	...
	{
		Logger::info("TURN", std::string(colorLabel(color)) + " has five in a row — breakable, " +
		                         colorLabel(opponentOf(color)) + " has one move to capture a pair");
