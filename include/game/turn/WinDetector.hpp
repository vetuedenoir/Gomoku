...
// `capturer` a-t-il une prise disponible, n'importe où sur le plateau ?
...
// Le cinq de `owner` est-il réfutable ? Une seule échappatoire : casser la
// ligne en prenant une paire qui en contient une pierre. Une prise ailleurs sur
...
// Nulle : le défenseur casse l'alignement ET la même prise le porte à
// CAPTURES_TO_WIN pierres. Les deux victoires tombent sur le même coup, aucune
// ne prime — la partie est nulle.
//
// Le butin d'un coup n'est pas borné à une paire : detect_captures cumule les
// huit demi-directions, donc un joueur à quatre pierres prises peut atteindre
// dix d'un seul coup. On ne teste donc pas un seuil sur le compteur d'avant, on
// compte les pierres réellement emportées par la prise.
//
// Plusieurs cinq peuvent passer par la case jouée : la prise ne fait nulle que
// si elle les casse TOUS, sinon un alignement survit et l'aligneur l'emporte.
...
// Cherche un cinq déjà posé appartenant à `owner`, sans connaître le coup qui
// l'a créé, et renvoie l'une de ses pierres comme ancre. Utilisé à la racine de
