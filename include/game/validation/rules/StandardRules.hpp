...
template<typename Traits> class StandardRules
{
public:
                                    ...
                                    const char** reason) const
{
	...
	if (get_bb_generic<Traits>(board.black, col, row) || get_bb_generic<Traits>(board.white, col, row))
	...
	typename Traits::Bitboard& moverPlane =
		const_cast<typename Traits::Bitboard&>((color == Color::Black) ? board.black : board.white);
	const typename Traits::Bitboard& oppPlane = (color == Color::Black) ? board.white : board.black;
