...
template<typename Traits> class StandardMovePolicy
{
public:
	bool isLegal(const GameBoard& board, int col, int row, Color color, const char** reason = nullptr) const
	{
...
private:
	static t_BWBoard<Traits> boardFrom(const GameBoard& board) { return GameBoard_to_bitboard<Traits>(board); }
