...
// Pierres à prendre pour gagner par capture.
...
struct Move
{
	...
	bool operator==(const Move& other) const noexcept { return col == other.col && row == other.row; }
};
...
struct t_cell
{
	...
	bool operator==(const t_cell& other) const { return x == other.x && y == other.y; }
};
...
template<typename Traits> struct TurnOutcome
{
	MoveResult                result;
	int                       capturesAdded;
...
template<typename T, std::size_t N> struct MoveList
{
	std::array<T, N> moves;
	std::size_t      count = 0;
	...
	void push(const T& m) noexcept { moves[count++] = m; }

	void pop() noexcept { --count; }

	T& top() noexcept { return moves[count - 1]; }

	const T& top() const noexcept { return moves[count - 1]; }

	T& operator[](std::size_t i) noexcept { return moves[i]; }

	const T& operator[](std::size_t i) const noexcept { return moves[i]; }
	...
	T* end() noexcept { return moves.data() + count; }

	const T* begin() const noexcept { return moves.data(); }
	const T* end() const noexcept { return moves.data() + count; }
};

// Étage de la chaîne de motifs (five → four → cross → three) sur lequel le score du coup s'est arrêtée.
