...
template<typename Traits> struct t_PatternList5
{
	CompactMask<Traits> masks[5];
	uint32_t            count;
};

template<typename Traits> struct t_super4
{
	CompactMask<Traits> mask;
	int16_t             hole_pos[2];
};

template<typename Traits> struct t_PatternList_super4
{
	t_super4<Traits> patterns[7];
	uint32_t         count;
};

template<typename Traits> struct t_Pattern4
{
	CompactMask<Traits> mask;
	int16_t             opposant_left;
	int16_t             opposant_right;
};

template<typename Traits> struct t_PatternList4
{
	t_Pattern4<Traits> patterns[4];
	int                count;
};

template<typename Traits> struct t_PatternGroup4
{
	CompactMask<Traits> masks[3];
	int16_t             hole_pos[3];
};

template<typename Traits> struct t_PatternList_Groupe4
{
...
// for openness checks; CompactMask stone sets replace full Bitboards (Lot 3).
template<typename Traits> struct t_PatternGroupe3
{
	int16_t stone_pos[4];
	int16_t hole_pos[2];
	int16_t oposant_pos[5];

	// stones at [0],[1],[2]  → SCORE_3_FULL
	CompactMask<Traits> fullMask{};

	// stones at [0],[2],[3] (hole at [1]) → SCORE_3_HOLE
	CompactMask<Traits> holeMask0{};

	// stones at [0],[1],[3] (hole at [2]) → SCORE_3_HOLE
	CompactMask<Traits> holeMask1{};
};

template<typename Traits> struct t_PatternList_Groupe3
{
...
typedef struct
{
	...
	int16_t right;
	int16_t middle;
	int16_t opposant_up[2];
	int16_t opposant_down[2];
	int16_t opposant_left[2];
	int16_t opposant_right[2];
} t_cross;
