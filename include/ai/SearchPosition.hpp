...
template<typename Traits> class SearchPosition
{
private:
	SearchPosition(t_BWBoard<Traits> board, uint64_t hash, const Color side, const ZobristHasher<Traits>& hasher);
...
public:
	...
	static SearchPosition fromBoard(const GameBoard& src, int capturesByBlack = 0, int capturesByWhite = 0);
	...
	int getCapturesForSideToMove() const { return getCapturesForColor(sideToMove()); }
	...
	int detect_and_hash_capture(const t_BWBoard<Traits>& board, int col, int row, const Color attackerColor,
	                            MoveStateHash& stateHash) const;

	void makeMove(int col, int row, Color color, MoveStateHash stateHash);
};

template<typename Traits> static Color toColor(CellStatus s)
{
...
template<typename Traits> void SearchPosition<Traits>::zoneApplyStone(int cx, int cy, int delta)
{
...
template<typename Traits> void SearchPosition<Traits>::zoneRebuild()
{
	_zoneMask = {};
	_zoneCount.fill(0);
...
template<typename Traits> typename Traits::Bitboard SearchPosition<Traits>::candidateMask() const
{
...
template<typename Traits>
SearchPosition<Traits> SearchPosition<Traits>::fromBoard(const GameBoard& src, int capturesByBlack, int capturesByWhite)
{
...
template<typename Traits> const ZobristHasher<Traits>& SearchPosition<Traits>::hasher()
{
	return ZobristHasher<Traits>::instance();
}

template<typename Traits> void SearchPosition<Traits>::undoMove(int col, int row, Color color)
{
...
template<typename Traits> const t_BWBoard<Traits>& SearchPosition<Traits>::board() const
{
	return _board;
}

template<typename Traits> uint64_t SearchPosition<Traits>::zobristHash() const
{
	return _hash;
}

template<typename Traits> Color SearchPosition<Traits>::sideToMove() const
{
	return _sideToMove;
}

template<typename Traits> void SearchPosition<Traits>::makeMove(int col, int row, Color color, MoveStateHash stateHash)
{
	...
	if (stateHash.state.whiteCaptures > 0)
	{
		...
		{
			clear_bit_generic<Traits>(_board.white, stateHash.state.capturedStones[k].x,
			                          stateHash.state.capturedStones[k].y);
	...
	else if (stateHash.state.blackCaptures > 0)
	{
		...
		{
			clear_bit_generic<Traits>(_board.black, stateHash.state.capturedStones[k].x,
			                          stateHash.state.capturedStones[k].y);
			zoneApplyStone(stateHash.state.capturedStones[k].x, stateHash.state.capturedStones[k].y, -1);
		}
	}
...
template<typename Traits>
int SearchPosition<Traits>::detect_and_hash_capture(const t_BWBoard<Traits>& board, int col, int row,
                                                    const Color attackerColor, MoveStateHash& stateHash) const
{
	...
	for (Direction dir : LINE_DIRS)
	{
		for (int sign : { -1, 1 })
		{
			    ...
			    get_bb_generic<Traits>(attacker, x3, y3))
			{
				...
				stateHash.state.capturedStones[captured]     = { static_cast<int_fast16_t>(x1),
					                                             static_cast<int_fast16_t>(y1) };
				stateHash.state.capturedStones[captured + 1] = { static_cast<int_fast16_t>(x2),
					                                             static_cast<int_fast16_t>(y2) };
				captured += 2;
			}
		}
	}
...
template<typename Traits> MoveStateHash SearchPosition<Traits>::buildMoveHash(EvaluatedMove move, Color color) const
{
	...
	if (caps > 0)
	{
		...
		for_each_captured_stone(move.captureMask, move.move.x, move.move.y,
		                        [&](int x, int y)
		                        {
									stateHash.state.capturedStones.push(
										{ static_cast<int_fast16_t>(x), static_cast<int_fast16_t>(y) });
