...
template<typename Traits> class MasterAI
{
...
public:
	...
	t_cell findBestMove(const SearchPosition<Traits>& position, Color color);
...
private:
	...
	void upgradeLightToFull(EvaluatedMove& move, const t_BWBoard<Traits>& board, Color color, int capturesBefore);
	...
	bool filterForcedReplies(const t_BWBoard<Traits>& board, MoveList<EvaluatedMove, MAX_BOARD_MOVES<Traits> >& ordered,
	                         Color mover);
	...
	void addDefenseToOrderingScore(EvaluatedMove& offense, const t_BWBoard<Traits>& board, Color side,
