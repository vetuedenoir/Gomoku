...
template<typename Traits> class MoveGenerator
{
public:
	...
	void generateEmptyMoves(const typename Traits::Bitboard&            candidateMask,
	                        MoveList<t_cell, MAX_BOARD_MOVES<Traits> >& movesArray) const;
...
MoveGenerator<Traits>::MoveGenerator(int activeZoneRadius) : _activeZoneRadius(activeZoneRadius)
{
}

template<typename Traits>
bool MoveGenerator<Traits>::isLegalMove(const t_BWBoard<Traits>& board, int col, int row, const Color color) const
{
...
template<typename Traits>
void MoveGenerator<Traits>::generateEmptyMoves(const typename Traits::Bitboard&            candidateMask,
                                               MoveList<t_cell, MAX_BOARD_MOVES<Traits> >& movesArray) const
{
