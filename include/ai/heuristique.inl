...
EvaluatedMove MasterAI<Traits>::rawShapeScoreV2(const t_BWBoard<Traits>& board, t_cell cell, Color color, int captureCount)
{
	...
	{
		data.score = score_open_three(r) + captureScore;
		// `caps` et non `captureScore` : ce dernier contient aussi les captures
		// déjà accumulées dans la partie, qui ne légalisent pas un double-trois.
...
// relancer detect_and_stock_capture. Il ne reste à traiter que les coups sortis
	...
	Color color, int captureCount)
{
	...
	move.score = cross_score(r) + captureCount + caps * CAPTURE_SCORE * 2;
