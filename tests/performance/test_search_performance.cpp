...
TEST_CASE("[PERF] findBestMove timing by depth on empty board")
{
	for (int depth : { 1, 2, 4, 6, 8 })
	{
		...
		LOG_INFO("PERF", "depth=" + std::to_string(depth) + " time=" + std::to_string(t.ms) + "ms" + " nodes=" +
		                     std::to_string(t.stats.nodesVisited) + " nps=" + std::to_string((long long)nps));
...
TEST_CASE("[PERF] findBestMove timing by depth on board from ASCII")
{
	...
	LOG_INFO("PERF", "depth=" + std::to_string(depth) + " time=" + std::to_string(t.ms) + "ms" +
	                     " nodes=" + std::to_string(t.stats.nodesVisited) + " nps=" + std::to_string((long long)nps));
...
TEST_CASE("[PERF][BENCH] move ordering: fixed positions by depth")
{
	...
	positions.push_back({ "menaces-croisees",
	                      boardFromAscii(
							  {
								  "...................",
								  ".......B...........",
	...
	positions.push_back({ "threes-opposes",
	                      boardFromAscii(
							  {
								  "...................",
								  "......BBB..........",
	...
	for (int depth : { 6, 8 })
	{
		...
		for (auto& p : positions)
		{
			...
			const long long pruningPct =
				t.stats.nodesVisited > 0 ? (t.stats.nodesPruned * 100LL / t.stats.nodesVisited) : 0;
			const long long nps = t.ms > 0 ? (t.stats.nodesVisited * 1000LL / t.ms) : 0;
			LOG_INFO("BENCH", std::string(variant) + " depth=" + std::to_string(depth) + " pos=" + p.name + " time=" +
			                      std::to_string(t.ms) + "ms" + " nodes=" + std::to_string(t.stats.nodesVisited) +
			                      " eval=" + std::to_string(t.stats.nodesEvaluated) +
			                      " pruned=" + std::to_string(t.stats.nodesPruned) + " (" + std::to_string(pruningPct) +
			                      "%)" + " ttHits=" + std::to_string(t.stats.ttHits) + " nps=" + std::to_string(nps));
		...
		LOG_INFO("BENCH", std::string(variant) + " depth=" + std::to_string(depth) +
		                      " TOTAL time=" + std::to_string(totMs) + "ms" + " nodes=" + std::to_string(totNodes) +
		                      " eval=" + std::to_string(totEval) + " pruned=" + std::to_string(totPruned) + " (" +
		                      std::to_string(totPct) + "%)" + " ttHits=" + std::to_string(totTtHits) +
