...
TEST_CASE("[Zobrist] buildMoveHash retire les clés des pierres capturées")
{
	...
	const uint64_t expected = posAfter.zobristHash() ^ H.sideKey() ^ H.captureHash(Color::White, 1);
	...
	const uint64_t withoutRemoval = msh.hash ^ H.key(6, 9, Color::White) ^ H.key(7, 9, Color::White);
	CHECK(withoutRemoval != expected);
	CHECK(withoutRemoval == (expected ^ H.key(6, 9, Color::White) ^ H.key(7, 9, Color::White)));
}
