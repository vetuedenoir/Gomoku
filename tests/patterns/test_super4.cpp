...
TEST_CASE("super4: horizontal B _ B B B _ B — detected from every cell in window")
{
	...
	{
		CHECK(check_super4_19(board.black, board.white, 3, 5) == 1);
	}
	SUBCASE("query from hole  at x=4")
	{
		CHECK(check_super4_19(board.black, board.white, 4, 5) == 1);
	}
	...
	{
		CHECK(check_super4_19(board.black, board.white, 6, 5) == 1);
	}
	SUBCASE("query from stone at x=7")
	{
		CHECK(check_super4_19(board.black, board.white, 7, 5) == 1);
	}
	SUBCASE("query from hole  at x=8")
	{
		CHECK(check_super4_19(board.black, board.white, 8, 5) == 1);
	}
	SUBCASE("query from stone at x=9")
	{
		CHECK(check_super4_19(board.black, board.white, 9, 5) == 1);
	}
}
