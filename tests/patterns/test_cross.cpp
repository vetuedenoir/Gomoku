...
TEST_CASE("cross: full cross queried from each arm position")
{
    ...
    SUBCASE("query from up arm   (9,8)")  { CHECK(check_cross_19(board.black, board.white, 9, 8)  == CROSS_FULL); }
    SUBCASE("query from left arm (8,9)")  { CHECK(check_cross_19(board.black, board.white, 8, 9)  == CROSS_FULL); }
    SUBCASE("query from centre   (9,9)")  { CHECK(check_cross_19(board.black, board.white, 9, 9)  == CROSS_FULL); }
    SUBCASE("query from right arm (10,9)"){ CHECK(check_cross_19(board.black, board.white, 10, 9) == CROSS_FULL); }
    SUBCASE("query from down arm (9,10)") { CHECK(check_cross_19(board.black, board.white, 9, 10) == CROSS_FULL); }
}
