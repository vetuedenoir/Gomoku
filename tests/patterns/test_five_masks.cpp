...
int find_masks(const BB19& stones, int x, int y, BB19* out,
               int maxOut = BitboardTool19::MAX_FIVE_MASKS)
{
...
TEST_CASE("five_masks: prefiltered and reference variants agree")
{
    ...
    for (int x = 0; x < 19; ++x)
    {
        for (int y = 0; y < 19; ++y)
        {
            ...
            const int nFast = patternTool().find_five_masks(
                board.black, x, y, fast, BitboardTool19::MAX_FIVE_MASKS);
            const int nRef = patternTool().find_five_masks_reference(
                board.black, x, y, ref, BitboardTool19::MAX_FIVE_MASKS);
